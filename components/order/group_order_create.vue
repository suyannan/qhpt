...
<script>
  import {get_default_address, creat_group_order, getPersonInfo,get_suit_count} from '../../api/index'
  ...
  export default {
    ...
    created() {
      ...
      //判断是否是重新发起的拼团
      if (this.$route.query.isNewGroup == 'true') {
    ...
    methods: {
          ...
          if (res.data.errcode == 0) {
            ...
            if (_this.pay_type == 'wei') {
              ...
              wx.chooseWXPay({
                ...
                success: function (res) {
                  ...
                  _this.$dialog.loading.close();
                  _this.$router.push('/main/group_invite?order_id=' + data.order_id);
                },
                cancel: function (res) {
                  _this.isPaying = false;
                  _this.$dialog.loading.close();
            ...
            _this.$dialog.confirm({
              ...
              opts: () => {
                ...
                sessionStorage.setItem('redirect', '/main/group_order_create?isNewGroup=' + _this.$route.params.isNewGroup);
                _this.$router.push({
                  path: '/unlogin',
                  query: {redirect: '/main/group_order_create?isNewGroup=' + _this.$route.params.isNewGroup}
                });
              }
            });
          } else {
      ...
      useVoucher() {
        ...
        this.$router.push({
          ...
          query: {
            order_price: this.goodsTotal,
            back: '/main/group_order_create?isNewGroup='+this.$route.query.isNewGroup
          }
        })
      },
      //选择收货地址
      chooseAddress() {
        ...
        this.$router.push({
          ...
          query: {back: '/main/group_order_create?isNewGroup='+this.$route.query.isNewGroup}
        })
      },
      //获取账户余额
      getAccount() {
